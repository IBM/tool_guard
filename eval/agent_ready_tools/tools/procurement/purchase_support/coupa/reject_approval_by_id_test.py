...
from agent_ready_tools.tools.procurement.purchase_support.coupa.reject_approval_by_id import (
    coupa_reject_approval_by_id,
)


def test_coupa_reject_approval_by_id() -> None:
    """Test that the `reject_approval_by_id` function returns the expected response."""
    ...
    with (
        patch(
            "agent_ready_tools.tools.procurement.purchase_support.coupa.reject_approval_by_id.get_coupa_client"
        ...
        response = coupa_reject_approval_by_id(test_approval_id).content

        # Ensure that reject_approval_by_id() executed and returned proper values
        assert response
        assert response.status == "rejected"

        # Ensure the API call was made with expected parameters
        mock_client.put_request.assert_called_once_with(
            resource_name=f"approvals/{test_approval_id}/reject"
        )
