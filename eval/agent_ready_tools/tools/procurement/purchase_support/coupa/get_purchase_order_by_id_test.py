...
from agent_ready_tools.tools.procurement.purchase_support.coupa.get_purchase_order_by_id import (
    coupa_get_purchase_order_by_id,
)


def mock_purchase_order_response() -> dict:
    """
    Mock response to use in other purchase order tests.

    Returns:
        The mock purchase order data.
    """
    ...
    return {
        ...
        "currency": {"code": "USD"},
        "total-with-estimated-tax": "2950.00",
...
def test_coupa_get_purchase_order_by_id() -> None:
    """Test that the `get_purchase_order_by_id` function returns the expected response."""
    test_data = mock_purchase_order_response()

    # Patch `get_coupa_client` to return a mock client
    with patch(
        "agent_ready_tools.tools.procurement.purchase_support.coupa.get_purchase_order_by_id.get_coupa_client"
        ...
        # Get purchase by ID
        response = coupa_get_purchase_order_by_id(purchase_order_id=test_data["id"]).content

        # Ensure that add_a_comment_to_invoice_coupa() executed and returned proper values
        assert response
        assert response.purchase_order_id == test_data["id"]
        assert response.requested_by == test_data["requisition-header"]["requested-by"]["fullname"]
        assert response.total_with_estimated_tax == test_data["total-with-estimated-tax"]
        assert len(response.order_lines.order_lines)
        assert response.shipping_address.postal_code == test_data["ship-to-address"]["postal-code"]

        requisition = response.requisition
        req_test_data = test_data["requisition-header"]
        assert requisition.id == 5194
        assert requisition.requested_by == req_test_data["requested-by"]["login"]
