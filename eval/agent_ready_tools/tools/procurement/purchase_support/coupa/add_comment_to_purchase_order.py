...
def coupa_add_comment_to_purchase_order(
    purchase_order_id: int,
    comment_text: str,
) -> ToolResponse[CoupaComment]:
    """
    Adds a comment to an existing purchase order in Coupa.

    Args:
        purchase_order_id: ID of the purchase order for the comment to be added to.
        comment_text: The comment text to add to the purchase order comments.
    ...
    response = client.post_request(
        resource_name=f"purchase_orders/{purchase_order_id}/comments", payload=payload
    )
