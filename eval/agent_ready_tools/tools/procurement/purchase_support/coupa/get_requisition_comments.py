...
def coupa_get_requisition_comments(requisition_id: int) -> ToolResponse[CoupaCommentList]:
    """
    Retrieves the comments of a requisition.

    Args:
        requisition_id: The id of the requisition.

    Returns:
        The retrieved list of comments of the requisition.
    """
    try:
        client = get_coupa_client()
    except (ValueError, AssertionError):
        return ToolResponse(success=False, message="Failure to retrieve credentials")
    response = client.get_request_list(resource_name=f"requisitions/{requisition_id}/comments")
    if len(response) == 0:
