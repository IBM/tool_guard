...
from agent_ready_tools.tools.productivity.box.get_file_details_by_name import (
    get_file_details_by_name,
)


def test_get_file_details_by_name() -> None:
    ...
    with patch(
        "agent_ready_tools.tools.productivity.box.get_file_details_by_name.get_box_client"
    ) as mock_box_client:
        ...
        response = get_file_details_by_name(name=test_data["file_name"]).details[0]

        # Ensure that get_file_details_by_name() executed and returned proper values
        assert response
        ...
        mock_client.get_request.assert_called_once_with(
            entity="search",
            params={"query": f"{test_data['file_name']}", "type": "file"},
        )
