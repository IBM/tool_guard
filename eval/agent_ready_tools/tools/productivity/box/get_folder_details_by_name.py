...
@dataclass
class FolderDetails:
    """Represents the details of folder in Box."""
...
class FolderDetailsResponse:
    """A list of folder details in a Box."""

    details: List[FolderDetails]


@tool(expected_credentials=BOX_CONNECTIONS)
def get_folder_details_by_name(name: str) -> FolderDetailsResponse:
    """
    Gets a list of folder details in this Box.

    Args:
        name: The name of the folder to filter in the Box.
    ...
    return FolderDetailsResponse(details=details_list)
