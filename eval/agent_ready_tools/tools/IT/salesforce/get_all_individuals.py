from typing import Optional

from ibm_watsonx_orchestrate.agent_builder.tools import ToolPermission, tool
from simple_salesforce import format_soql  # type: ignore[attr-defined]

from agent_ready_tools.clients.salesforce_client import get_salesforce_client
from agent_ready_tools.tools.IT.salesforce.salesforce_schemas import Individual
from agent_ready_tools.utils.sql_utils import format_where_input_string
from agent_ready_tools.utils.tool_credentials import SALESFORCE_CONNECTIONS


@tool(
    permission=ToolPermission.READ_ONLY,
    expected_credentials=SALESFORCE_CONNECTIONS,
)
def get_all_individuals(search: Optional[str] = None) -> list[Individual]:
    """
    Returns a list of Individual objects from the Salesforce org, filtered based on the provided
    query parameters.

    Args:
        search: The SQL where clause from LLM (to clean up).

    Returns:
        A list of Individual objects.
    """
    client = get_salesforce_client()
    cleaned_clause = format_where_input_string(search or "")

    rs = client.salesforce_object.query_all_iter(
        format_soql(f"SELECT Id, Name, OwnerId FROM Individual {cleaned_clause}")
    )
    results: list[Individual] = []
    for row in rs:
        individual_id = row.get("Id")
        name = row.get("Name")
        owner_id = row.get("OwnerId")

        results.append(Individual(id=individual_id, name=name, owner_id=owner_id))
    return results
